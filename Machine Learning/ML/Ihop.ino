...
void runInferenceAndSendResult() {
    ...
    // Send classification result via BLE
    String classificationResult;
    for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
        if (result.classification[ix].value > 0.8) {  // Assuming threshold for "positive" detection
            classificationResult = result.classification[ix].label;
            break;  // Send the most confident result
        }
    }

    // Fallback if no high confidence classification is found
